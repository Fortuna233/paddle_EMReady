...
class WMSA(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, type):
        ...
        self.n_heads = input_dim // head_dim #向下取整，多头注意力
        self.window_size = window_size
        self.type = type
        self.embedding_layer = nn.Linear(self.input_dim, 3*self.input_dim, bias=True)

        self.relative_position_params = nn.Parameter(
            torch.zeros((2 * window_size - 1) * (2 * window_size - 1) * (2 * window_size - 1), self.n_heads))
    ...
    def generate_mask(self, h, w, d, p, shift):
        attn_mask = torch.zero(h, w, d, p, p, p, p, p, p, dtype=torch.bool, device=self.relative_position_params.device)
        if self.type == 'W':
            return attn_mask
    ...
    def forward(self, x):
        if self.type != 'W':
            x = torch.roll(x, shifts=(-(self.window_size // 2), -(self.window_size // 2), -(self.window_size // 2)), dim = (1, 2, 3))
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x += self.drop_path(self.msa(self.ln2(x)))
        return x
...
class SCUnet(nn.Module):
    def __init__(self, in_nc=1, config=[2, 2, 2, 2, 2, 2, 2], dim=32, drop_path_rate=0.2, input_resolution=48, head_dim=16, window_size=3, n_classes=1):
        ...
        begin += config[3]
        self.m_up3 = [nn.ConvTranspose3d(8*dim, 4*dim, 2, 2, 0, bias=False),] + [ConvTransBlock(2*dim, 2*dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW', input_resolution//4) for i in range(config[4])]

        begin += config[4]
        self.m_up2 = [nn.ConvTranspose3d(4*dim, 2*dim, 2, 2, 0, bias=False),] + [ConvTransBlock(dim, dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW', input_resolution//2) for i in range(config[5])]
